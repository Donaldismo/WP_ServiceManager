...
class WtServiceManagerAdmin_
{
	function __construct(){
		...
		add_action( 'manage_service_posts_custom_column', array($this, 'max_column_display'), 10, 2 );
	...
	function post_type_service_add_options_box() {
			add_meta_box('wp_service_options', 'Дополнительные настройки', array($this, 'service_options_html'), 'service', 'normal', 'high');
	}

	/* HTML код блока */
	function service_options_html($post) {
		echo '<p><label>Стоимость (в рублях)<input type="number" name="service_options[price]" value="' . 
		get_post_meta($post->ID, 'price', 1) . 
		'" style="width:50%" /></label></p>';
		echo '<p><label>Максимальное количество<input type="number" name="service_options[max]" value="' . 
		get_post_meta($post->ID, 'max', 1) . 
		'" style="width:50%" /></label></p>';
		//echo '<script>document.getElementById("postdivrich").remove();document.getElementById("edit-slug-box").remove();</script>';
    ...
    //вывод максимального количества
	function max_column_display( $column_name, $post_id ) {
		if ( 'max' != $column_name) return;
		 
		$price = get_post_meta($post_id, 'max', true);
